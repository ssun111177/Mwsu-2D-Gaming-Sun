var play = {
	...
	update: function () {
		...
		if (frame_counter % 90 == 0) {
			...
			this.spanwpmball(game.global.pmball_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 5)), has_given_point = true)
			else
			this.spawnstar(game.global.star_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 5)), has_given_point = true)
		}

		if (game.global.score > 3 && frame_counter % 80 == 0) {
			if(Math.random() < 0.3)
			...
			this.spanwpmball(game.global.pmball_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 10)), has_given_point = true)
			else
		...
		if (game.global.score > 5 && frame_counter % 70 == 0) {
			if(Math.random() < 0.3)
			this.spawndiamond(game.global.diamond_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 20)), has_given_point = true)
			else if(Math.random() < 0.3)
			this.spanwpmball(game.global.pmball_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 20)), has_given_point = true)
			else
			this.spawnstar(game.global.star_id++, Math.random() * game.width , game.height, speed = (200 + (game.global.score * 20)), has_given_point = true)
		}
	...
	spanwpmball: function (entity, x, y, speed, has_given_point) {
	...
	explodeObstacle: function (bullet, obstacle) {
		...
		var exploing = this.game.add.sprite(obstacle.body.x,obstacle.body.y,"explode")

		exploing.animations.add("explode");
		exploing.animations.play("explode");
		//set the position of explode
		exploing.anchor.setTo(0.5,0.5);
		...
		//points increase if score increas
		if(game.global.score < 3)
			game.global.score += 1;
		else if(game.global.score < 5)
			game.global.score += 2;
		else
			game.global.score += 3;
	},
